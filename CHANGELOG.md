...
### Changed
- Replace APIs deprecated in Unity 6.6. `AppDomain.GetAssemblies` (UAC0005) now goes through
  `PackageDetector.GetLoadedAssemblies`, which uses `CurrentAssemblies` on 6000.6+, and
  `AssetDatabase.ImportPackage` (CS0618) becomes `AssetPackage.Package.Import` on 6000.6+. Older
  Editors keep the previous calls.
